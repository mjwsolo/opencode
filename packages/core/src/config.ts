...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Load order inside a directory (later wins): compatibility `opencode.*` names, then `localcode.*`.
    const names = ["opencode.json", "opencode.jsonc", "localcode.json", "localcode.jsonc"]
    const dirs = [".opencode", ".localcode-agent"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [...dirs.toReversed(), ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => dirs.includes(path.basename(item)))
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => !dirs.includes(path.basename(item))).toReversed()
    ...
    // global config, project files, then `.localcode-agent` / `.opencode` files.

...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("localcode mode stack is not registered for this keymap")
  return value
}
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  ...
  return createMemo<CommandSlashEntry[]>(() =>
    entries().flatMap((entry) => {
      // Only explicitly named user commands belong in the slash menu.
      const slashName = typeof entry.command.slashName === "string" ? entry.command.slashName : ""
      if (!slashName) return []

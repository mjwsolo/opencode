...
export const { use: useEditorContext, provider: EditorContextProvider } = createSimpleContext({
  ...
  init: (props: { integration?: EditorIntegration; WebSocketImpl?: typeof WebSocket }) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.LOCALCODE_EDITOR_SSE_PORT || process.env.OPENCODE_EDITOR_SSE_PORT
    ...
    const connect = () => {
      ...
      current.addEventListener("open", () => {
        ...
        request("initialize", {
          ...
          capabilities: {},
          clientInfo: { name: "localcode", version: "0.0.0" },
        })
      })

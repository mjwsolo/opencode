...
// localcode is local-only. Upstream opencode proxied unmatched server requests
// to the hosted web UI (app.opencode.ai) when no embedded UI was bundled; that
// fallback is removed. Only the embedded UI is served; otherwise 404. The
// `client` service is accepted (unused) so the server route wiring is unchanged.

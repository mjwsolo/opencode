...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall localcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g localcode-agent",
      pnpm: "pnpm uninstall -g localcode-agent",
      bun: "bun remove -g localcode-agent",
      yarn: "yarn global remove localcode-agent",
      brew: "brew uninstall localcode",
      choco: "choco uninstall localcode",
      scoop: "scoop uninstall localcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "localcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".localcode-agent") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using localcode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes("# localcode") ||
      content.includes(".localcode-agent/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# localcode" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (
        trimmed.includes(".localcode-agent/bin") ||
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(".localcode-agent/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".localcode-agent") || trimmed.includes(".opencode")))
    ) {
      continue
    }

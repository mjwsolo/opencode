...
describe("HttpApi UI fallback", () => {
  // localcode is local-only: with no embedded UI available the server must
  // return 404 rather than proxy the request to the hosted opencode web UI.
  it.live("does not proxy to an upstream web UI when the embedded UI is unavailable", () =>
    Effect.gen(function* () {
      let proxiedUrl: string | undefined
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      ...
      // Auth passes; no embedded UI and no upstream proxy means 404.
      expect(response.status).toBe(404)
    }),
  )
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      ...
      // Auth passes; no embedded UI and no upstream proxy means 404 (not 401).
      expect(response.status).toBe(404)
    }),
  )

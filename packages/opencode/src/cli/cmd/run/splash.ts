...
//
// Renders the localcode entry mark and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    const mark = logo.left.slice(1).map((row) => row.slice(0, 9))  // compact "LO" mark from the wordmark
    const top = 1
    ...
    push(lines, body_left, top, "localcode", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `localcode --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )

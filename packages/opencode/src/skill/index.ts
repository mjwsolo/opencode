...
// Built-in skill that ships with localcode. The model's intuition for what a
// localcode.json should look like is often wrong, and localcode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch localcode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_LOCALCODE_SKILL_NAME = "customize-localcode"
const CUSTOMIZE_LOCALCODE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating localcode's own configuration: localcode.json, localcode.jsonc (or legacy opencode.json), files under .localcode-agent/ (or legacy .opencode/), or files under ~/.config/localcode-agent/. Also use when creating or fixing localcode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring localcode itself."
const CUSTOMIZE_LOCALCODE_SKILL_BODY = SkillPlugin.CustomizeLocalcodeContent
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_LOCALCODE_SKILL_NAME] = {
          name: CUSTOMIZE_LOCALCODE_SKILL_NAME,
          description: CUSTOMIZE_LOCALCODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_LOCALCODE_SKILL_BODY,
        }

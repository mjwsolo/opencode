...
// etc.).
//
// The visual language mirrors the localcode app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `localcode is now connected to ${escapeHtml(provider)}.` : "localcode is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `localcode couldn't finish connecting to ${escapeHtml(provider)}.`
        : "localcode couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from localcode.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · localcode</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("localcode couldn't finish connecting to "+PROVIDER+"."):"localcode couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from localcode."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("localcode is now connected to "+PROVIDER+"."):"localcode is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .brand .wordmark { font-size: 1.125rem; font-weight: 600; letter-spacing: 0.02em; color: var(--oc-icon-strong); }
...
// localcode wordmark: plain text, styled with the same tokens as the page.
const WORDMARK = `<span class="wordmark" role="img" aria-label="localcode">localcode</span>`

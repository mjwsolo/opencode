...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors theme/assets/localcode.json, the default theme) since the
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied — paste it into a localcode bug report."
                : "Copy the report and file a localcode bug report to help us fix this."}
            </text>
            <text fg={colors.muted}>localcode {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildReport(message: string, stack: string) {
  // Plain Markdown so it can be pasted into any bug tracker or chat. Cap the
  // stack so a runaway trace does not overflow the clipboard.
  const MAX_STACK_LENGTH = 6000
  const body = stack.length > MAX_STACK_LENGTH ? stack.slice(0, MAX_STACK_LENGTH) + "\n… (truncated)" : stack
  return [
    `The localcode TUI crashed with an unexpected error.`,
    ``,
    `**Version:** localcode ${InstallationVersion}`,
    `**OS:** ${describeOS()}`,

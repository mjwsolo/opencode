...
it.instance("discovered aliases have independent vision capabilities across immediate switches", () =>
  Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      path.join(directory, "localcode.json"),
      JSON.stringify({
        provider: {
          localcode: {
    ...
    let status = { current: "vision-model", state: "ready", vision: true, ctx: 131072 }
    ...
    status = { current: "text-model", state: "ready", vision: false, ctx: 10500 }
    const second = yield* provider.getModel(ProviderV2.ID.make("localcode"), ModelV2.ID.make("text-model"))
    expect(second.limit.context).toBe(10500)
    expect(second.limit.output).toBe(2625)
    expect(first.limit.context).toBe(131072)
    expect(second.capabilities.input.image).toBe(false)
    expect(second.capabilities.attachment).toBe(false)
    expect(first.capabilities.input.image).toBe(true)
    status = { current: "text-model", state: "ready", vision: true, ctx: 131072 }
